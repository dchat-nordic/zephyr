...
static int bmi270_reg_read(uint8_t reg, uint8_t *data, uint16_t length,
...
static int bmi270_reg_write(uint8_t reg, const uint8_t *data, uint16_t length,
		     const struct device *dev)
{
	const struct bmi270_config *cfg = dev->config;
...
static int bmi270_reg_write_with_delay(uint8_t reg,
				       const uint8_t *data,
				       uint16_t length,
				       const struct device *dev,
				       uint32_t delay_us)
{
	...
	ret = bmi270_reg_write(reg, data, length, dev);
	if (ret == 0) {
		k_usleep(delay_us);
	}
	return ret;
			     ...
			     const struct device *dev)
{
	...
	if (odr || osr) {
		ret = bmi270_reg_read(BMI270_REG_ACC_CONF, &acc_conf, 1, dev);
		if (ret != 0) {
			return ret;
		}

		ret = bmi270_reg_read(BMI270_REG_PWR_CTRL, &pwr_ctrl, 1, dev);
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (odr || osr) {
		ret = bmi270_reg_write(BMI270_REG_ACC_CONF, &acc_conf, 1, dev);
		if (ret != 0) {
			return ret;
		}
		...
		ret = bmi270_reg_write_with_delay(BMI270_REG_PWR_CTRL,
						  &pwr_ctrl, 1, dev,
			   ...
			   const struct device *dev)
{
	...
	ret = bmi270_reg_read(BMI270_REG_ACC_RANGE, &acc_range, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	acc_range = BMI270_SET_BITS_POS_0(acc_range, BMI270_ACC_RANGE,
					  reg);
	ret = bmi270_reg_write_with_delay(BMI270_REG_ACC_RANGE, &acc_range, 1,
					  dev, BMI270_INTER_WRITE_DELAY_US);
			    ...
			    const struct device *dev)
{
	...
	if (odr || osr) {
		ret = bmi270_reg_read(BMI270_REG_GYR_CONF, &gyr_conf, 1, dev);
		if (ret != 0) {
			return ret;
		}
	...
	if (odr || osr) {
		ret = bmi270_reg_write(BMI270_REG_GYR_CONF, &gyr_conf, 1, dev);
		if (ret != 0) {
			return ret;
		}
			  ...
			  const struct device *dev)
{
	...
	ret = bmi270_reg_read(BMI270_REG_GYR_RANGE, &gyr_range, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	ret = bmi270_reg_write_with_delay(BMI270_REG_GYR_RANGE, &gyr_range, 1,
					  dev, BMI270_INTER_WRITE_DELAY_US);
...
static int8_t write_config_file(const struct device *dev)
{
	...
	for (index = 0; index < sizeof(bmi270_config_file);
		...
		ret = bmi270_reg_write_with_delay(BMI270_REG_INIT_ADDR_0,
						  addr_array, 2, dev,
						  BMI270_INTER_WRITE_DELAY_US);

		if (ret == 0) {
			ret = bmi270_reg_write_with_delay(BMI270_REG_INIT_DATA,
			       ...
			       enum sensor_channel chan)
{
	...
	ret = bmi270_reg_read(BMI270_REG_ACC_X_LSB, buf, 12, dev);
	if (ret == 0) {
...
static int bmi270_attr_set(const struct device *dev,
			   enum sensor_channel chan,
			   ...
			   const struct sensor_value *val)
{
	...
	if ((chan == SENSOR_CHAN_ACCEL_X) || (chan == SENSOR_CHAN_ACCEL_Y)
		...
		case SENSOR_ATTR_SAMPLING_FREQUENCY:
			ret = set_accel_odr_osr(val, NULL, dev);
			break;
		case SENSOR_ATTR_OVERSAMPLING:
			ret = set_accel_odr_osr(NULL, val, dev);
			break;
		case SENSOR_ATTR_FULL_SCALE:
			ret = set_accel_range(val, dev);
			break;
		default:
			ret = -ENOTSUP;
		}
		...
		switch (attr) {
		case SENSOR_ATTR_SAMPLING_FREQUENCY:
			ret = set_gyro_odr_osr(val, NULL, dev);
			break;
		case SENSOR_ATTR_OVERSAMPLING:
			ret = set_gyro_odr_osr(NULL, val, dev);
			break;
		case SENSOR_ATTR_FULL_SCALE:
			ret = set_gyro_range(val, dev);
			break;
		default:
			ret = -ENOTSUP;
		}
...
static int bmi270_init(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Could not get pointer to %s device", dev->name);
		return ret;
	}
	...
	ret = bmi270_reg_read(BMI270_REG_CHIP_ID, &chip_id, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	ret = bmi270_reg_write(BMI270_REG_CMD, &soft_reset_cmd, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	ret = bmi270_reg_read(BMI270_REG_PWR_CONF, &adv_pwr_save, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	ret = bmi270_reg_write_with_delay(BMI270_REG_PWR_CONF, &adv_pwr_save, 1,
					  dev, BMI270_INTER_WRITE_DELAY_US);
	if (ret != 0) {
		return ret;
	}
	...
	ret = bmi270_reg_write(BMI270_REG_INIT_CTRL, &init_ctrl, 1, dev);
	if (ret != 0) {
		return ret;
	}
	...
	for (tries = 0; tries <= BMI270_CONFIG_FILE_RETRIES; tries++) {
		ret = bmi270_reg_read(BMI270_REG_INTERNAL_STATUS, &msg, 1, dev);
		if (ret != 0) {
			return ret;
		}
